...
int main( int argc, char *argv[] )
{
    ...
    for(i = 0; i < n_threads; i++) {
        ...
        ta[i]->section = i;
        ta[i]->section_width = (double)image_height/n_threads;
        ta[i]->max = max;
...
void * compute_image(void * arg) {
    ...
    double start = t->section*t->section_width;
    double end = (t->section+1)*t->section_width;

    // make sure we go all the way to the end
    if(height - end < t->section_width) end = height;
...
{
    int color = 255*i/max;
    return MAKE_RGBA(color,color,color,0);
}
